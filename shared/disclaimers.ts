 ...
 *
 * MOVED from `leanshot/src/lib/disclaimers.ts` in Phase 4 D-04 so both the
 * browser-side UI (chart watermark, doctor report) and the Edge Function
 * system prompt (`supabase/functions/ai-chat/system-prompt.ts`) consume the
 * same copy. The browser wrapper at `leanshot/src/lib/disclaimers.ts`
 * re-exports from here; the Edge Function imports via the `shared/disclaimers`
 * alias in `supabase/functions/import_map.json`.
 *

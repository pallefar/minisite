...
| 44-01-02 | 01 | 0 | COMMUNITY-01/02/06 | T-44-01 | RLS tests: cross-tenant, tier-gate, reaction idempotency | tsc | `cd /Users/karstenhaldan/minisite/leanshot && npx tsc --noEmit -p tsconfig.json` | ⬜ Wave 0 | ⬜ pending |
...
| 44-04-01 | 04 | 1 | COMMUNITY-04 | T-44-02 | mux-create-upload tier gate (Free=403, Pro/Lifetime/Trial=200) | Deno test | `cd /Users/karstenhaldan/minisite && $HOME/.deno/bin/deno test --no-check supabase/functions/mux-create-upload/index.test.ts` | ⬜ Wave 1 | ⬜ pending |
| 44-04-02 | 04 | 1 | COMMUNITY-04 | T-44-03 | mux-webhook HMAC verify + asset.ready/asset.errored handlers | Deno test | `cd /Users/karstenhaldan/minisite && $HOME/.deno/bin/deno test --no-check supabase/functions/mux-webhook/index.test.ts` | ⬜ Wave 1 | ⬜ pending |
...
| 44-06-01 | 06 | 1 | COMMUNITY-01/02 | T-44-05 | CommunityPost (sanitize render + tombstone + edited marker) + CommentThread + ReactionBar (toggle RPC) | tsc | `cd /Users/karstenhaldan/minisite/leanshot && npx tsc --noEmit -p tsconfig.json` | ⬜ Wave 1 | ⬜ pending |
...
| 44-09-01 | 09 | 2 | COMMUNITY-05/06 | T-44-01, T-44-06 | use-space-realtime + tier-gated SpaceList + SpaceView | tsc | `cd /Users/karstenhaldan/minisite/leanshot && npx tsc --noEmit -p tsconfig.json` | ⬜ Wave 2 | ⬜ pending |
| 44-09-02 | 09 | 2 | COMMUNITY-04/05/06 | T-44-01b | vite.config.ts sub-chunk rules (media + mentions BEFORE community-feed catch-all); admin SpaceEditor; staff-write RLS migration; App.tsx route | bundle grep | `cd /Users/karstenhaldan/minisite/leanshot && grep -q 'community-media' vite.config.ts && grep -q 'community-mentions' vite.config.ts && ls supabase/migrations/20270720000006_p44_community_spaces_admin_policies.sql 2>/dev/null` | ⬜ Wave 2 | ⬜ pending |
...
- [ ] `supabase/migrations/20270720000006_p44_community_spaces_admin_policies.sql` — admin INSERT/UPDATE policies on community_spaces (44-09 follow-up)
...
- [ ] `leanshot/tests/rls/community-spaces-rls.test.ts` — cross-tenant + global-space SELECT proofs

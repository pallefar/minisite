...
- **D-17:** Sentry coverage = **Capacitor SDK + native Sentry Cocoa + native Sentry Android** (max coverage). dSYM upload via Sentry CLI in fastlane. Reuses existing Sentry project from Phase 1 with separate releases tagged `ios@<version>` / `android@<version>`.

...
- **D-04:** Cross-org isolation = **platform admin sees all; clinic admin sees only their org's reports**. RLS predicate on community_reports SELECT: `public.is_staff() OR exists(select 1 from public.org_members om join public.events|community_spaces resolution where … role='admin' and (resolved_org_id matches target's space.org_id))`. Practically: a helper SECDEF function `public.can_moderate_report(report_row community_reports) returns boolean` resolves the target's parent space → org_id and checks org membership with role='admin'. Phase 28 `org_members` is the source of truth. Clinic admin moderates patients in their own clinic-private spaces; platform staff have global override.
...
- **D-15:** Ban enforcement = **`supabase.auth.admin.signOut(user_id)` + RLS deny on user-owned writes**. On `user_moderation_state.status='banned'` INSERT/UPDATE, AFTER trigger calls `pg_net.http_post` to a new `ban-enforcement` Edge Fn which:
  1. Invokes Supabase Auth Admin API to revoke all active sessions for the user (`supabase.auth.admin.signOut(user_id)`).

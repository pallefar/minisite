# Phase 59: Apple OAuth + Onboarding Completion â Research
...
**Confidence:** HIGH (core web path, existing codebase); MEDIUM (native Capacitor Apple Sign-In â no existing entitlement or plugin in the repo)
...
### Locked Decisions
- Reuse Phase 34 `isAppleEnabled()` gate + `AuthCallbackView` â do NOT rebuild. Surface the "Sign in with Apple" button on login, signup, AND onboarding goal-step, behind the gate.
...
- Live Apple OAuth provider config + flag-flip + on-device sign-in + private-relay live E2E â Phase 70.
- Mobile Lighthouse â¥90 on-device measurement + PostHog live ship-winner â Phase 70.
- Google / other OAuth providers â out of scope (Apple-only this phase).
</user_constraints>
...
|----|-------------|------------------|
| AUTH-07 | Apple OAuth provider configured in Supabase Auth Dashboard (VENDOR-01 pending) â Phase 59 ships code-complete; live config defers to P70 | `signInWithOAuthProvider('apple')` call-site already present in `ConsumerOnboardingRenderer`; `AuthCallbackView` handles callback |
| AUTH-08 | "Sign in with Apple" button on login + signup + onboarding surfaces; â¥44px tap target; native Apple branding compliance | Button exists in `ConsumerOnboardingRenderer` (onboarding surface); MISSING in `SignInForm` + `SignUpForm`; branding rules documented below |
...
| AUTH-10 | ONBOARD-05/06/07/10/11 finished â activation walkthrough fixtures, admin step builder fixtures, Mobile Lighthouse re-verify | Activation event fully wired (record-activation Fn + fireActivation hooks). Gaps: admin fixture seeding (superadmin row); Lighthouse run blocked on vendor config. Admin step builder `OnboardingBuilderModule` already ships; needs fixture data to run HITL walkthrough |
...
5. Mobile Lighthouse measurement: `npm run lighthouse:onboard` script already exists; the plan needs a task that runs it and asserts â¥90 (deferred run to post-vendor-config in P70, but the test infrastructure must be verified as runnable).
...
| Private-relay email handling | Database / Storage (trigger + profiles table) | â | `handle_new_user()` trigger creates profile from `id` only â email field is nullable in profiles |
...
| i18n button labels | Browser / Client (react-i18next + locale JSON) | â | Static strings in public/locales; check-locale-coverage.sh enforces coverage |
...
|---------|----------|-----|----------|-------------|-----------|-------------|
| `@capacitor-community/apple-sign-in` | npm | 5.9 yrs (2020-06-16) | 20 | github.com/capacitor-community/apple-sign-in | slopcheck unavailable â [ASSUMED] | Approved (verified on npm, official Capacitor Community org, no postinstall scripts, 5.9 yr history) |
| `posthog-js` | npm | Already installed | â | github.com/PostHog/posthog-js | slopcheck unavailable â [ASSUMED] | Approved (official PostHog SDK, already in production) |
| `@supabase/supabase-js` | npm | Already installed | â | github.com/supabase/supabase-js | slopcheck unavailable â [ASSUMED] | Approved (official Supabase SDK, already in production) |
...
SignInWithApple.authorize()     supabase.auth.signInWithOAuth
  (native Apple dialog)            { provider: 'apple' }
       |                                |
identityToken (JWT from Apple)   redirect â /auth/callback
       |                                |
supabase.auth.signInWithIdToken  AuthCallbackView.exchangeCodeForSession
  { provider:'apple', token }          |
       ...
       |_______________________|
        ...
        â INSERT INTO profiles(id) ON CONFLICT DO NOTHING
                    |
         AuthCallbackView checks profiles.completed_onboarding_at
                    |
           â '#/dashboard' OR '#/onboarding'
                    |
...
â   âââ SignInForm.tsx          # ADD Apple OAuth button (gated)
â   âââ SignUpForm.tsx          # ADD Apple OAuth button (gated)
âââ components/onboarding/
â   âââ ConsumerOnboardingRenderer.tsx  # Wire PostHog experiment variant
âââ lib/auth.ts                 # ADD signInWithAppleNative() for iOS path
âââ lib/native/apple-sign-in.ts # NEW: thin wrapper over @capacitor-community/apple-sign-in

apps/ios/App/App/
âââ App.entitlements            # ADD com.apple.developer.applesignin entitlement
...
**Critical:** Apple does NOT provide the user's real email on subsequent sign-ins â only on the FIRST sign-in. The `result.response.email` may be null or a private relay address. The profile creation must handle this (see Pitfall 1).
...
```tsx
// Source: posthog-js API [ASSUMED â posthog-js 1.376.x, verified installed]
...
const steps: StepId[] = useMemo(() => {
  ...
  try {
    ...
    // posthog not yet initialized â fall through to default
  }
  return DEFAULT_STEPS;
}, []); // no deps needed â flag value is stable post-init
```

**Note:** `posthog.getFeatureFlag(key)` returns the variant string or `undefined` if the flag doesn't exist/isn't loaded. It is synchronous after `posthog.onFeatureFlags()` resolves. In tests, mock `posthog.getFeatureFlag` to return a specific variant. The actual flag key name is Claude's Discretion â `'onboarding_flow_variant'` is the suggested name.
...
- **Adding `isAppleEnabled()` check at the button as a boolean JSX condition without a re-render trigger:** The `isAppleEnabled()` function reads `localStorage` which is not reactive. It's called at render time, which is sufficient â do not wrap in `useState`/`useEffect` unless the gate needs to respond to live localStorage changes (it doesn't; the flag is set at deploy time).
- **Emitting `activation_completed` from the client:** The event is `server_only` (D-05). Only `record-activation` Edge Fn emits it. `fireActivation()` in `src/lib/onboarding/activation-hooks.ts` calls the Edge Fn â do not bypass.
...
|---------|-------------|-------------|-----|
| Apple identity token verification | Custom JWT verification against Apple public keys | `supabase.auth.signInWithIdToken({ provider: 'apple', token })` | Apple key rotation, nonce verification, expiry â all handled by GoTrue |
...
| Activation event deduplication | Client-side `localStorage` flag | `record-activation` Edge Fn (already ships) | The Fn uses a DB advisory lock + `activated_at IS NULL` guard â client can't replicate this safely |
...
**Answer:** `@capacitor-community/apple-sign-in` is the correct approach. The web OAuth path (`supabase.auth.signInWithOAuth`) does NOT satisfy Apple's App Store requirement for native iOS apps â Apple mandates `ASAuthorizationAppleIDProvider` (the native dialog), not a webview. The plugin is NOT yet a dependency in `package.json` (only `@capacitor-community/admob` is installed). It must be added. The native path is gated behind `detectPlatform() === 'ios'` inside the `isAppleEnabled()` guard. The bridge calls `supabase.auth.signInWithIdToken({ provider: 'apple', token: identityToken })` â Supabase v2 supports this natively. [VERIFIED: npm registry â package exists at 7.1.0, 5.9 yr history, no postinstall scripts]
...
### Q2: Web Sign-in-with-Apple â existing path is complete
...
**Apple HIG button requirements (AUTH-08):** [ASSUMED â from Apple HIG documentation knowledge, not verified via live URL in this session]
- Minimum tap target: â¥44pt (already enforced by `min-h-[44px]` on all auth buttons in this codebase)
- Apple logo: must use the official Sign in with Apple button logo (the black Apple logo). Options: (a) use CSS + SVG Apple logo from Apple's Sign in with Apple button assets, (b) use the SF Symbol `apple.logo` via a native bridge on iOS. For web, Apple provides a JavaScript library (`appleid.cdn-apple.com/appleauth/static/jsapi/appleid/1/en_US/appleid.auth.js`) but Supabase OAuth makes it unnecessary â the Supabase-provided button styling is sufficient if it meets size requirements.
...
- The wordmark must read "Sign in with Apple" (exact Apple-specified string) â the existing locale key `"continue_apple": "Continue with Apple"` does NOT match Apple's required wordmark. This is a potential HIG violation. The exact required text is "Sign in with Apple" not "Continue with Apple". [ASSUMED â Apple HIG requirement; recommend planner flags this for decision]

### Q3: Apple private-relay email handling

**Answer:** The `handle_new_user()` trigger (in `supabase/migrations/20261101000001_profiles_is_staff.sql`) creates the profile row on `auth.users INSERT` with only `id` â no email dependency whatsoever: `INSERT INTO public.profiles (id) VALUES (new.id) ON CONFLICT DO NOTHING`. The profile is thus created correctly for any auth method including private-relay. [VERIFIED: codebase grep â migration confirmed]

When Apple provides a private-relay email (`username@privaterelay.appleid.com`), Supabase stores it in `auth.users.email`. The `profiles` table has no `email` column (per `reference_profiles_email_vs_auth_users_email` â `profiles` only has `id`, `is_staff`, `created_at`, `completed_onboarding_at`, `primary_goal`, `locale`, `timezone`, `admin_role`, etc.). No code change is needed for profile creation to work with private-relay emails.

**Activation event:** `record-activation` requires only a valid JWT (user must be authenticated). It reads `profiles.created_at` to compute `days_since_signup` and uses `profiles.id` as the key. No email is consulted. Private-relay users activate identically to regular users. [VERIFIED: codebase â `supabase/functions/record-activation/index.ts` confirmed]
...
**Answer:** The merge helper is fully built. `ConsumerOnboardingRenderer` already fires it (lines 182-233): when `signedInUserId` transitions from `null` to a UUID, it reads the `_ls_anon` cookie, calls `merge-anon-session` Edge Fn with `{ cookie_ids: [cookie], anon_distinct_id: posthog.get_distinct_id() }`, and calls `clearAnonCookie()` in the `finally` block. [VERIFIED: codebase â ConsumerOnboardingRenderer.tsx lines 180-233]

The merge also works for Apple OAuth: `AuthCallbackView` exchanges the code â session â redirects to `#/onboarding` if `completed_onboarding_at IS NULL`. `ConsumerOnboardingRenderer` mounts, detects the signed-in user, fires the merge. No additional wiring needed for Apple specifically.

**Gap:** For users who sign in with Apple directly via `SignInForm` or `SignUpForm` (not via the onboarding surface), the merge `useEffect` in `ConsumerOnboardingRenderer` will not fire because those forms don't render `ConsumerOnboardingRenderer`. If the user had an anon session before signing in from the login page, the merge is missed. This is a pre-existing limitation for Google OAuth too. Resolution options: (a) fire the merge in `AuthCallbackView` after successful session exchange, or (b) document as known limitation. Recommend option (a) â add best-effort merge call in `AuthCallbackView` after the profile check, consistent with D-08.

### Q5: PostHog Experiments wiring

**Existing infrastructure (fully built in Phase 34):**
- `OnboardingABPanel` (admin) â lists PostHog experiments tagged "onboarding", shows rollout %, lets superadmin ship winner
- `ship-winner-flag` Edge Fn â calls PostHog API to set flag to 100% rollout for a variant; requires `POSTHOG_PERSONAL_API_KEY` (VENDOR-09, deferred to P70)
- `onboarding-funnel-query` Edge Fn â fetches experiment list from PostHog; requires same key
- Both Fns return `{ error: 'vendor_unconfigured', service: 'posthog' }` gracefully when the key is unset

**Gap (Phase 59 deliverable):** `ConsumerOnboardingRenderer` reads `DEFAULT_STEPS` unconditionally â the PostHog flag is not actually consulted (the `useMemo` at lines 136-139 returns `DEFAULT_STEPS` in both branches). Phase 59 must wire `posthog.getFeatureFlag('onboarding_flow_variant')` (or whichever key is used) into the `steps` computation.
...
**Live ship-winner verification** requires `POSTHOG_PERSONAL_API_KEY` and `POSTHOG_PROJECT_ID` set as Supabase Function Secrets â defer to Phase 70 per CONTEXT.md locked decisions.

### Q6: ONBOARD-05/06/07/10/11 â current state
...
|-------------|-------------|-------------------|
| ONBOARD-05: Activation event fires after first real action | Complete â `record-activation` Fn + `fireActivation` hooks + `activation_completed` event | Automated-verify-only (fixtures missing for admin walkthrough) |
| ONBOARD-06: Activation event instrumented in TAXO registry + measured per cohort | Complete â `activation_completed` in `src/lib/analytics/events.ts` at line 105; `record-activation` captures via PostHog server-side | PostHog key needed for live verification |
| ONBOARD-07: Admin drag-and-drop step builder | Complete â `OnboardingBuilderModule` ships with `SortableTreePanel` | HITL walkthrough deferred (needs superadmin fixture row) |
| ONBOARD-10: Mobile Lighthouse â¥90 | Script exists (`npm run lighthouse:onboard`) | Run not yet completed â carries from 34-10 Task 4 |
| ONBOARD-11: Anon-to-auth merge race safety | Complete â `merge-anon-session` with advisory lock | Automated tests pass; HITL on two-device race deferred to milestone UAT |

**Phase 59 deliverable for AUTH-10:** (a) Seed a superadmin fixture row so the 34-08 HITL walkthrough can complete. This is a one-time `INSERT INTO auth.users ... + profiles.admin_role='superadmin'` via Supabase CLI. (b) Run `npm run lighthouse:onboard` and assert â¥90. If â¥90: close ONBOARD-10. If <90: document score and defer optimization to Phase 70. (c) These two items are now the ONLY remaining gaps for AUTH-10.

### Q7: i18n â current state and new keys needed

**Existing keys:** `onboarding:consumer.auth.continue_apple` exists in both `public/locales/en/onboarding.json` and `public/locales/es/onboarding.json` (verified via file read â ES: `"Continuar con Apple"`). [VERIFIED: codebase]
...
**Why it happens:** Apple only delivers the user's email once â on initial authorization. Subsequent `SignInWithApple.authorize()` calls return `email: null`.
...
**How to avoid:** Add the entitlement to `App.entitlements` AND enable the capability in Xcode â Signing & Capabilities â "Sign in with Apple". The CI build (Fastlane + match) must use a provisioning profile that includes this capability (Phase 70 vendor step).
...
### Pitfall 5: SignInForm/SignUpForm use inline English strings â adding the Apple button without i18n wiring fails CI
...
**How to avoid:** The project uses `@capacitor/core: ^8.3.4` (Capacitor 8). Plugin v7.1.0 targets Capacitor 7+ per the plugin's semver convention. Verify compatibility at install time. [ASSUMED â versioning convention, not verified against plugin's exact peerDependencies at research time]
...
**Why it happens:** Per `reference_deno_test_top_level_serve_trap` memory â project Edge Fns use `Deno.serve()` not guarded by `import.meta.main`.
...
|----------|-------------|------------------|
| Stored data | `profiles` rows for existing users â no change needed; trigger handles all new auth methods | None |
| Live service config | Apple provider in Supabase Auth Dashboard â NOT configured; flag kept OFF | Phase 70 vendor step |
| Live service config | `POSTHOG_PERSONAL_API_KEY` + `POSTHOG_PROJECT_ID` â NOT set as Supabase Function Secrets | Phase 70 vendor step (VENDOR-09) |
| OS-registered state | iOS Capacitor entitlement `com.apple.developer.applesignin` â NOT in `App.entitlements` | Code edit (add XML entry) |
...
|------------|------------|-----------|---------|----------|
| Node.js | npm install, vitest | â | v22.18.0 | â |
| npm | package installs | â | bundled | â |
| Deno | Edge Fn tests | Assumed â | `$HOME/.deno/bin/deno` | â |
| Supabase CLI | db push, functions deploy | â | v2.98.2 | â |
| Playwright | e2e tests | â | ^1.59.1 | â |
| Vitest | unit tests | â | ^4.1.5 | â |
| Lighthouse CLI (`@lhci/cli`) | `npm run lighthouse:onboard` | â | ^0.15.1 | â |
| `@capacitor-community/apple-sign-in` | Native iOS Apple Sign-In | â (not installed) | 7.1.0 available | None â must install |
| Apple Developer Portal | entitlement provisioning | N/A (human-only) | â | Phase 70 |
| Supabase Apple Auth provider | live Apple OAuth | N/A (VENDOR-01 pending) | â | Flag stays OFF; code ships ready |
| PostHog Personal API key | ship-winner-flag | N/A (VENDOR-09 pending) | â | vendor_unconfigured degraded mode |
| Physical iOS device | on-device Apple Sign-In test | N/A | â | Defer to Phase 70 UAT |
...
- Apple Auth provider, PostHog key â both have graceful degradation paths already implemented.
...
| Config file | `leanshot/vitest.config.ts` (has `projects:` block â per `reference_vitest_4_projects_config_masks_default` memory, plain `npm test` may collect 0 tests; use `npx vitest run --config vite.config.ts` or `npm run test:unit`) |
...
|--------|----------|-----------|-------------------|-------------|
| AUTH-08 | Apple button renders when `isAppleEnabled()=true` in SignInForm | unit | `npx vitest run src/components/auth/__tests__/SignUpForm.test.tsx` | Partial â `SignUpForm.test.tsx` exists; need Apple variant |
| AUTH-08 | Apple button absent when `isAppleEnabled()=false` | unit | same | Same |
| AUTH-08 | Apple button renders in ConsumerOnboardingRenderer when gated | unit | `npx vitest run src/components/onboarding/ConsumerOnboardingRenderer.test.tsx` | â exists, likely has Apple gate tests from P34 |
| AUTH-09 | Profile created for private-relay email (trigger test) | manual/E2E | N/A â requires live Supabase | Manual at Phase 70 |
| AUTH-10 | Activation event fires after first action | unit | `npx vitest run src/lib/onboarding/` | â activation-hooks already tested |
| AUTH-10 | Admin step builder renders Builder tab | unit | `npx vitest run src/components/admin/onboarding-builder/OnboardingBuilderModule.test.tsx` | â exists |
| AUTH-10 | Lighthouse â¥90 on onboarding route | Lighthouse | `npm run lighthouse:onboard` | â script exists |
| AUTH-11 | OnboardingABPanel shows vendor_unconfigured state | unit | `npx vitest run src/components/admin/onboarding-builder/OnboardingABPanel.test.tsx` | â exists |
| AUTH-11 | PostHog experiment variant routes to treatment steps | unit | `npx vitest run src/components/onboarding/ConsumerOnboardingRenderer.test.tsx` | â Wave 0 gap |
| AUTH-11 | PostHog variant defaults to control when flag undefined | unit | same | â Wave 0 gap |

### Wave 0 Gaps
- [ ] `src/components/onboarding/ConsumerOnboardingRenderer.test.tsx` â add test: "renders treatment_a steps when PostHog flag returns 'treatment_a'"
- [ ] `src/components/onboarding/ConsumerOnboardingRenderer.test.tsx` â add test: "falls back to DEFAULT_STEPS when PostHog getFeatureFlag returns undefined"
- [ ] `src/components/auth/SignInForm.test.tsx` â NEW FILE: test Apple button visibility under `isAppleEnabled()` gate (SignInForm has no test file, only SignUpForm does)
- [ ] `src/lib/native/apple-sign-in.test.ts` â NEW FILE: unit test for `signInWithAppleNative()` (non-iOS platform returns `native_apple_ios_only` error)
...
| V5 Input Validation | yes | `identityToken` from Apple plugin passed directly to Supabase â no user-editable input in the auth flow itself |
...
| `apple_disabled` error surfaced as auth flow | Information Disclosure | Return value `{ error: { message: 'apple_disabled' } }` â no Supabase call is made; no information about GoTrue config leaks |
...
> RESOLVED in planning: (1) HIG wordmark -- new `sign_in_apple` key = "Sign in with Apple" (App-Store-mandated), not reuse of continue_apple. (2) Merge scope -- best-effort anon-merge added in AuthCallbackView (Plan 59-03 T3), scoped to the just-authenticated session. (3) Lighthouse timing -- `node --check` runnability this phase; ≥90 on-device defers to P70.
...
1. **Apple HIG wordmark compliance**
   ...
   - Recommendation: Planner should add a task to decide: keep existing key OR add `auth:sign_in_with_apple` + `auth:iniciar_sesion_con_apple` to both locale files. For the native iOS path (native dialog from `@capacitor-community/apple-sign-in`), Apple renders its own button â no wordmark choice needed there.
...
3. **Lighthouse run timing**
   - What we know: `npm run lighthouse:onboard` runs against `http://localhost:5173/onboard`. Phase 59 must verify â¥90.
   - What's unclear: Whether the current codebase (post-Phase 58 i18n bundle additions) still achieves â¥90.
...
- Codebase: `src/lib/auth.ts` â `isAppleEnabled()`, `signInWithOAuthProvider()` implementation verified
- Codebase: `src/components/auth/AuthCallbackView.tsx` â PKCE callback handler verified
- Codebase: `src/components/onboarding/ConsumerOnboardingRenderer.tsx` â existing Apple button, merge useEffect verified
- Codebase: `supabase/migrations/20261101000001_profiles_is_staff.sql` â `handle_new_user()` trigger creates profile from `id` only, verified
- Codebase: `supabase/functions/record-activation/index.ts` â activation event architecture verified
- Codebase: `supabase/functions/merge-anon-session/index.ts` â merge architecture verified
- Codebase: `public/locales/en/onboarding.json` + `public/locales/es/onboarding.json` â `continue_apple` key verified in both
- npm registry: `@capacitor-community/apple-sign-in@7.1.0` â package verified, no postinstall scripts, 5.9 yr history
- npm registry: `posthog-js@1.376.0`, `@supabase/supabase-js@2.106.2` â versions verified

### Secondary (MEDIUM confidence)
- `leanshot/.planning/milestones/v1.3-phases/34-m2-onboarding-overhaul-activation-event/34-CARRY-OVER.md` â confirmed which Phase 34 items remain deferred
- `leanshot/.planning/phases/59-apple-oauth-sign-in-with-apple-onboarding-completion/59-CONTEXT.md` â scope constraints and locked decisions

### Tertiary (LOW / ASSUMED confidence)
- Apple HIG wordmark requirement ("Sign in with Apple" exact string) â [ASSUMED] from training knowledge; should be verified against Apple's Human Interface Guidelines
- `@capacitor-community/apple-sign-in` TypeScript API shape (`SignInWithApple.authorize()` params + `response.identityToken` field name) â [ASSUMED] from plugin's npm description and category knowledge; verify against plugin's TypeScript definitions at install time
...
- Standard Stack (web path): HIGH â all verified in codebase
- Standard Stack (native path): MEDIUM â package verified on npm, API shape assumed from training
- Architecture: HIGH â fully reconstructed from codebase read
- Pitfalls: HIGH (Apple-specific) / MEDIUM (HIG wordmark, Capacitor version compatibility)
- ONBOARD-05..11 state: HIGH â CARRY-OVER.md + codebase read confirmed exactly what's missing

**Research date:** 2026-05-26
**Valid until:** 2026-06-25 (stable â Supabase Auth and Capacitor APIs change slowly; PostHog JS is minor-patched frequently but API surface is stable)

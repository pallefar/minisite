...
| TBD | TBD | TBD | GDPR-01 | T-22-DSAR | DSAR export bundle includes Postgres + Storage + Stripe + PostHog + affiliate (hashed for others); 30-day SLA enforced via `dsar_requests` | Deno + bundle assertion | `(cd supabase/functions && deno test dsar-export)` | ❌ W0 | ⬜ pending |
| TBD | TBD | TBD | GDPR-02 | T-22-CONSENT | EU visitor: Analytics off by default; PostHog `init()` only fires after `acceptedService('posthog','analytics')` | Playwright | `npx playwright test cookie-consent.spec.ts` | ❌ W0 | ⬜ pending |
| TBD | TBD | TBD | GDPR-03 | T-22-REDACT | Other users' emails in affiliate_ledger/audit_logs are SHA-256 hashed in DSAR bundle | unit + Deno | `npm test -- dsar-redaction.test.ts` | ❌ W0 | ⬜ pending |

...
- **Defer the StripeConnectOnboardingCard render** to a child slot — 19-06b ships the card. For 19-06a, render a placeholder div with comment `{/* Plan 19-06b inserts StripeConnectOnboardingCard here via PartnerContext sub-provider OR PartnerDashboard imports it from the sibling plan's path */}`. CONCRETE INTEGRATION: PartnerDashboard imports `<StripeConnectOnboardingCard>` from `@/components/partner/StripeConnectOnboardingCard` (file created in 19-06b). Build will fail until 19-06b ships; both plans land in Wave 3 in the same execute-phase batch.

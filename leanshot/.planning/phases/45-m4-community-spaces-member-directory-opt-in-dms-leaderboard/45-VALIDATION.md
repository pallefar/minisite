...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 45-01-01 | 01 | 0 | COMMUNITY-07/08/09 | T-45-01 | Schema migration ships 11 profiles cols + 5 new tables + leaderboard_handle (RESEARCH correction) | migration grep | `grep -c "ADD COLUMN IF NOT EXISTS" supabase/migrations/20270727000001_p45_schema.sql` ≥ 12 | ❌ Wave 0 | ⬜ pending |
...
| 45-07-01 | 07 | 2 | COMMUNITY-07/08/09 | T-45-01 | Zustand sub-view dispatch + use-dm-inbox-realtime hook | TS compile + grep | `cd leanshot && npx tsc -p tsconfig.app.json --noEmit && grep -c "activeCommunityView" src/lib/store.ts` ≥ 4 | ❌ Wave 0 | ⬜ pending |
| 45-07-02 | 07 | 2 | COMMUNITY-07/09 | T-45-05, T-45-15 | Directory + ProfileCard + LeaderboardChip + ReportButton + Settings toggles ship; no new DOMPurify | TS compile + grep | `cd leanshot && npx tsc -p tsconfig.app.json --noEmit && grep -rE "new DOMPurify\\(|createDOMPurify\\(" src/components/community/CommunityDirectoryView.tsx src/components/community/ProfileCard.tsx src/components/community/LeaderboardChip.tsx src/components/community/ReportButton.tsx` returns 0 lines | ❌ Wave 0 | ⬜ pending |
| 45-07-03 | 07 | 2 | COMMUNITY-08 | T-45-04, T-45-05 | DM inbox + thread + composer; vite chunks BEFORE community-feed catch-all; bundle budget passes | build + script | `cd leanshot && npm run build && bash scripts/assert-bundle-budget.sh` exits 0 AND chunk-ordering awk check passes | ❌ Wave 0 | ⬜ pending |

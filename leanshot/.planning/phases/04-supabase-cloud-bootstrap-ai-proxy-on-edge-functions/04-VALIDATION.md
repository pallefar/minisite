...
| 04-01-03 | 01 | 1 | PROD-07 | T-04-06 | Function secrets (MOONSHOT_API_KEY=placeholder + MOONSHOT_MODEL=kimi-k2-latest) set per `04-ADDENDUM-MOONSHOT.md`; .env.secrets deleted; no commit contains a real key | smoke | `npx --prefix leanshot supabase secrets list \| grep -cE 'MOONSHOT_(API_KEY\|MODEL)' \| grep -q '^[[:space:]]*2$' && test ! -f supabase/.env.secrets` | ✅ | ⬜ pending |

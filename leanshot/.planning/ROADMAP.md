...
- [ ] **Phase 71: Product Updates & Centralized Changelog** — First feature phase after the launch gate. Single source of truth for "What's New": an admin "Push Updates" authoring UI (`/admin/product-updates`) that publishes `changelog_entries` which surface identically in-app on web/iOS/Android (Capacitor shares the SPA drawer) AND auto-populate App Store Connect + Play "What's new" release notes via a fastlane-metadata sync script. Extends the existing Phase-42 changelog infra (table + RLS + WhatsNewDrawer + useChangelog); additive migration (version, draft/published/archived status, created_by) + draft-hidden RLS. Decided 2026-05-30: BOTH in-app + store notes; admin UI first; AdMob out of scope. REQ-IDs PU-01..PU-04.
  - **Plans:** 2 plans (2 waves)
    - [ ] 71-01-PLAN-admin-push-updates.md — Admin "Push Updates" module + additive status/version/created_by schema + draft-hidden RLS + published-only useChangelog filter (PU-01/02/03)
    - [ ] 71-02-PLAN-store-notes-sync.md — Centralized changelog → App Store + Play release-notes sync script (markdown→plain-text) wired before the fastlane upload lanes (PU-04)

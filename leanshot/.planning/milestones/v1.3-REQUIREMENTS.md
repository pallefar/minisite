...
audit_path: .planning/milestones/v1.3-MILESTONE-AUDIT.md
roadmap_archive: .planning/milestones/v1.3-ROADMAP.md
---

# Milestone v1.3 Requirements (Archived)

**Status:** SHIPPED 2026-05-25 (audit: tech_debt — no functional gaps).

## Requirement outcomes

- **Validated (functionally satisfied):** 206/206 REQ-IDs land on shipped owner-phases — confirmed via audit's 3-source cross-reference (REQUIREMENTS.md checkbox, SUMMARY frontmatter, ROADMAP owner-phase claim) and 7 sampled cross-phase integration flows (all pass). See `v1.3-MILESTONE-AUDIT.md`.
- **Approved automated-verify-only (HUMAN-UAT deferred to staging):** 33 signals consolidated across 7 phases (35:6, 36:3, 40:4, 41:6, 43:4, 44:4, 51:6). Runbooks in `.planning/v1.3-uat-deferred.md` + `41-VERIFICATION.md` + `51-CARRY-OVER.md`. Phase 41 + 51 REQ-IDs (EMBED-01..08, POLISH-10, TRAFFIC-01..12) carry vendor-secret + browser-UAT residual obligations.
- **Deferred to v1.4 (vendor/scope-blocked):**
  - **Phase 32-06 (Spanish contractor delivery)** — I18N-04/05/06/09 partial. ES corpus, KB ES content, clinical glossary, TRANSLATOR-WORKFLOW.md outstanding.
  - **Phase 34-08 + 34-10 Apple/PostHog checkpoints** — ONBOARD-02 (Apple OAuth wiring), ONBOARD-05/06/07/10/11 partial pending fixtures; magic-link + Google live.
  - **Phase 50 (RAG knowledge base) Waves 2-4** — only Wave 1 data layer + admin shell + event registry shipped (50-01..04). Plans 50-05..09 (summarize/queue/embeddings/AI-coach citations/Tip-of-day/newsletter/public hub) carry to v1.4. No dedicated REQ-IDs blocked — Phase 50 is decision-driven (36 D-IDs in 50-CONTEXT.md).
  - **Phase 42 — 5 device-UAT signals** — axe-core CI baseline + push device smoke + dark-mode VR snapshots + PWA installability.
- **Vendor secrets deferred (operator-runbook at staging):** Better Stack, Calendly OAuth (5 secrets), Sentry CSP report-uri, traffic-recorder env, `SHARE_TOKEN_SECRET`, `QUARTERLY_NPS_SIGNING_KEY`, `POSTHOG_PERSONAL_API_KEY`+`PROJECT_ID`, `SLACK_WEBHOOK_EXPERIMENTS_URL`. See audit §"Operator-runbook deferrals".

> Bulk requirement checkboxes below are toggled to `[x]` at milestone close to match `audit.requirements: 206/206 functionally satisfied`. The 102/206 stale-`[ ]` drift documented in the v1.3 audit is the source of this bulk toggle; per-phase precision lives in SUMMARY frontmatter and the audit cross-reference table.
...
- [x] **ADMIN-01**: Admin opens any module in the admin shell from a modular router (Users / Content / Onboarding / Gamification / Reviews / Membership / Analytics / AI / Helpdesk / Billing / Settings / Audit Log) without hard-coded conditionals — each module is feature-flagged, route-gated, lazy-loaded
- [x] **ADMIN-02**: Admin views the audit log with a before/after JSONB diff viewer; rows are append-only with RLS denying updates/deletes
- [x] **ADMIN-03**: Admin login enforces 2FA (Supabase Auth TOTP) for any Admin-or-higher role; bypass blocked at middleware
- [x] **ADMIN-04**: Admin runs bulk actions on the Members table (CSV export, tag, comp-plan, ban, force-password-reset) with operation-confirmation modal + audit-log entry per row affected
- [x] **ADMIN-05**: Admin defines + saves cohorts via a cohort-builder UI ("free users >7d", "past-due >3 days", "clinic admins") with cross-feature reuse (cohorts wired into TAXO, GAME challenges, PAYWALL variants, RECOMMEND triggers, SAVE eligibility)
- [x] **ADMIN-06**: Admin invokes the command palette (Cmd+K) for fuzzy-search navigation across modules + recent items + quick actions

### WS2 — Foundation: Event Taxonomy + Server-side PostHog (TAXO, 6 REQ-IDs)

- [x] **TAXO-01**: Canonical event registry versioned in repo at `src/lib/analytics/events.ts`; each event annotated `{name, version, payload-schema, phi: boolean}`; PR-reviewed (no ad-hoc events allowed via ESLint rule)
- [x] **TAXO-02**: Server-side PostHog capture via `_shared/posthog-server.ts` Edge Function helper for events adblockers eat (signup, payment, activation, refund); `client.shutdown()` called before Edge return
- [x] **TAXO-03**: Cohort builder primitives wired (`cohort_definitions` table + `cohort_membership` matview refreshed via pg_cron 15-min) — consumed by ADMIN-05
- [x] **TAXO-04**: PostHog session-replay masks PHI via `disable_session_recording_on_url` regex covering `/clinic/*`, `/admin/*`, `/share/*`, `/auth/*`; data-sentry-mask attributes on PHI components
- [x] **TAXO-05**: Anomaly-detection cron flags funnel-drop regressions in admin (rolling 7-day baseline; alert when conversion < baseline-2σ on any tracked funnel)
- [x] **TAXO-06**: Event-version migration tooling (event-name + payload migrations versioned; downgrade-map for stale clients during rolling deploy)

### WS3 — A Revenue: Multi-Tier Affiliate (AFFTIER, 6 REQ-IDs)

- [x] **AFFTIER-01**: Affiliate progresses through tiered commissions (Standard / Gold / Lifetime) with volume-threshold promotion (Standard → Gold at N paid-conversions; Gold → Lifetime by admin grant); locked-once-earned policy enforced
- [x] **AFFTIER-02**: Every new affiliate_conversion row stamps `tier_at_conversion_time` + `commission_cents` at insert; commission NEVER recomputed retroactively on tier upgrade
- [x] **AFFTIER-03**: Partner dashboard shows tier-progress bar + commissions earned-per-tier breakdown + next-tier-thresholds
- [x] **AFFTIER-04**: Lifetime affiliates receive monthly recurring commissions on existing subscribers via `affiliate-lifetime-recurring` cron Edge Fn (until subscriber cancels)
- [x] **AFFTIER-05**: Click-rate anomaly detector (impressions/clicks ratio Z-score >3σ on 7-day baseline) flags conversions to admin queue; extends v1.2 AFF-08 hybrid scope
- [x] **AFFTIER-06**: Co-branded `/r/{code}/landing` template selection by tier (Gold partners get premium-styled landing; reuses Phase 19 AffiliateLandingResolver)
...
- [x] **EMBED-01**: Admin drops Calendly block on a landing page → visitor sees Calendly booking widget in sandboxed iframe + lazy-loaded after cookie consent
- [x] **EMBED-02**: Admin drops YouTube block → visitor sees YouTube embed via `youtube-nocookie.com` (privacy-preserving) + lazy-loaded
- [x] **EMBED-03**: Admin drops Tally block → visitor sees Tally form embed with per-form configuration + sandboxed + consent-gated
- [x] **EMBED-04**: Every embed iframe has `sandbox` attribute with minimum-required permissions + CSP allowlist updated for each provider's hosts + dompurify XSS protection on any admin-pasted HTML
- [x] **EMBED-05**: Embed loading shows DS Skeleton until iframe `onLoad` fires; opacity transitions 0→1 over 200ms (gated by `useReducedMotion()`); per Phase 15 pattern
- [x] **EMBED-06**: Embed blocks render in helpdesk KB articles (extends EMBED-01..04 reach to M6 surfaces)
- [x] **EMBED-07**: Admin drops Custom-iframe block (paste-your-own-iframe URL) with admin allowlist of hostnames per-deployment; allowlist enforced via CSP + iframe `src` validator
- [x] **EMBED-08**: Admin sees live preview of Calendly availability inline in PageEditor (editor-side OAuth bouncing handled via popup, not iframe-internal; per V13-EMBED pitfall)
...
- [x] **I18N-01**: User sees Spanish UI via `?lang=es` query string (NOT `/es/` path prefix — avoids Vercel rewrite doubling per V13-9 + ARCHITECTURE)
- [x] **I18N-02**: First-visit Accept-Language detection + cookie set; explicit user preference persisted to `profiles.locale` overrides browser default
- [x] **I18N-03**: react-i18next + i18next-http-backend lazy-loads `/locales/{lng}/{ns}.json` (NOT eager-bundled JSON imports)
- [x] **I18N-04**: Transactional emails ship Spanish templates (welcome series, password reset, payment receipt, clinic invite, dunning, DSAR confirmation, lifecycle behavior triggers)
- [x] **I18N-05**: KB articles store Spanish translations at `kb_articles WHERE locale='es'` (admin uploads parallel `{slug}.es.md` files; full-text search HELP-KB-search respects locale)
- [x] **I18N-06**: Crowdin glossary workflow (or in-house equivalent) established for translator pipeline; translator workflow documented; review/approval gates wired
- [x] **I18N-07**: ICU pluralization correctness verified (ES has more plural forms than EN — singular/plural/zero/other); icu-format `{count, plural, ...}` strings tested per pluralization rule
- [x] **I18N-08**: Admin-editable locale overrides (`locale_overrides` table) — admin hot-patches translation bugs per-org or per-deployment without redeploying or waiting for Crowdin
- [x] **I18N-09**: Medical-term clinical glossary review pass before Spanish ships (clinical-content-liability gate); GLP-1 drug names, dosing terms, symptom names reviewed by Spanish-native clinical advisor
- [x] **I18N-10**: CSS uses logical properties (`margin-inline-start`, `padding-block-end`) per LTR-RTL future-proofing (no RTL languages in v1.3 scope but prepared for v1.5)

### WS10 — C B2B: Clinic Organizations (ORG, 13 REQ-IDs)

- [x] **ORG-01**: Schema: `organizations` + `org_members` + `org_invites` + `org_subscriptions` + `org_settings` + `org_branding` + `org_patient_links` + `org_consent_grants` + 4 more downstream tables (16+ migrations)
- [x] **ORG-02**: JWT `app_metadata.org_ids` claim propagates org-scope to all RLS policies (336ms propagation window — UI loading state handled per `reference_supabase_app_metadata_jwt_propagation`)
- [x] **ORG-03**: `withOrgScope` service_role wrapper enforces `org_id` filter on EVERY service_role query (compile-time TypeScript enforcement; runtime assertion + Sentry alert if bypassed) — closes V13-2 service_role-bypass vector
...
- [x] **ORG-05**: Every org-scoped table gets a live cross-tenant impersonation proof test (extends Phase 5/6/19/22 project rule from `user_id` axis to `org_id` axis)
- [x] **ORG-06**: `src/lib/org.ts` org-context layer detects current org via path + member.org_id + provides surface-check helpers + overlays white-label theme tokens
...
- [x] **CLIN-01**: Clinic admin configures per-clinic ranking weights (which signals matter most for patient roster ranking: dose-adherence / weight-loss / activity / etc.) stored in `org_settings` JSONB
- [x] **CLIN-02**: Dose-trend cron job runs nightly; compares each patient's dose-pattern against per-clinic thresholds; inserts `clinician_alerts` row when threshold breached
- [x] **CLIN-03**: Clinician receives in-app notification + email when alert fires; email is PHI-aware (uses HIPAA-eligible email path; never includes patient name in subject)
- [x] **CLIN-04**: Alert delivery is debounced (same alert within 24h aggregates to one notification) + retry-on-failure (3 attempts over 1h)
- [x] **CLIN-05**: Admin views aggregate alert metrics in clinic-side dashboard (alerts-by-type, alerts-by-clinician-ack-rate)
- [x] **CLIN-06**: Clinician-side alert acknowledge + snooze workflow; alerts age out (auto-resolve after 7d if not acted on)
- [x] **CLIN-07**: Per-patient threshold overrides (clinician sets patient-specific dose-trend thresholds via patient drill-in surface)
- [x] **CLIN-08**: Aggregate clinic dashboard surfaces population-level dose-trend metrics ("# patients on Wegovy below dosing range this week") via materialized view
...
- [x] **ONBOARD-02**: User signs in via magic link OR Google OAuth OR Apple OAuth (password optional, not required); native input types + ≥44px tap targets on mobile
...
- [x] **ONBOARD-05**: Onboarding ends by completing one real task (logged injection / scheduled dose / joined challenge based on stated goal) — activation event fires
- [x] **ONBOARD-06**: Activation event clearly defined for LeanShot + instrumented in TAXO event registry + measured per cohort
- [x] **ONBOARD-07**: Admin drag-and-drop step builder (question type / copy / validation / branching) — schema stored as JSON in `onboarding_flows.config`
...
- [x] **ONBOARD-10**: Mobile Lighthouse score ≥90 on onboarding route
- [x] **ONBOARD-11**: Anonymous-to-authenticated session merge handles race conditions (two devices both opening anonymous sessions); merge strategy chooses richest-data row
...
- [x] **GAME-01**: User accumulates XP in append-only `xp_ledger` (entry per qualifying action: log injection +5, complete weekly challenge +50, etc.); level computed deterministically from XP total
- [x] **GAME-02**: Streak tracked in `streak_state` (current_streak_days + longest_streak_days + last_action_at) computed via daily `pg_cron` job respecting user's stored timezone
- [x] **GAME-03**: Freeze tokens granted free monthly (1/month) to all users; ethical-only mechanic (NOT monetized, NOT sold); appendable via admin grant for support cases
- [x] **GAME-04**: Cohort-scoped opt-in leaderboards (anonymized handles) refreshed via `pg_cron` 15-min on materialized view
- [x] **GAME-05**: Weekly challenges admin-configurable (challenge_type + duration + reward) stored in `weekly_challenges`; ethical-only loss aversion (notify BEFORE streak break, no dark patterns)
- [x] **GAME-06**: Progress rings render on dashboard for goals + streak (Site-rotation card already shipped in v1.2 DS-9); `useReducedMotion` respected
- [x] **GAME-07**: Shareable level-up cards (OG-image generation via Vercel Function); user shares to Twitter/X/LinkedIn/Instagram with auto-generated visual
- [x] **GAME-08**: Weekly challenges support admin variant A/B (cohort-scoped challenge framing variants; e.g., A: "Log 5 injections this week" vs B: "Build a 5-day streak")
- [x] **GAME-09**: Cross-streak rewards (streak + challenge completion combo unlocks special badge); celebrates compound consistency

### WS15 — User-facing: M3 Review Prompt Engine (REVIEW, 8 REQ-IDs)

- [x] **REVIEW-01**: Internal NPS-style rating fires as INDEPENDENT surface (NOT gating any store-native prompt) — V13-3 plan-checker BLOCKER active on conditional native-prompt code
- [x] **REVIEW-02**: Admin defines trigger rules via rule-builder UI (if/and/or composition over event names + cohort membership + cooldown state)
- [x] **REVIEW-03**: Cooldown rules enforced: min days between prompts (default 30); max prompts per lifetime (default 3)
- [x] **REVIEW-04**: Promoter (4-5★ NPS) routes to external CTA (Trustpilot / G2 / Capterra); user opt-in to external review redirect
- [x] **REVIEW-05**: Non-promoter (1-3★ NPS) routes to in-app feedback form → auto-creates M6 helpdesk ticket (subject "Feedback from NPS rating")
- [x] **REVIEW-06**: PostHog A/B on prompt copy + timing (test variants of trigger conditions, copy framings, CTA wordings)
- [x] **REVIEW-07**: Admin views per-funnel review-rate dashboard (prompt shown → internal rating → external review posted) with per-variant breakdown
- [x] **REVIEW-08**: Multi-channel external CTAs (Trustpilot / G2 / Capterra / Apple-PWA-store / Google-Play-PWA when applicable)

### WS16 — User-facing: M6 Helpdesk Core (HELP, 13 REQ-IDs)

- [x] **HELP-01**: Schema: `tickets` + `ticket_messages` + `ticket_attachments` + `ticket_tags` + `kb_articles` + `kb_article_versions` + `csat_responses` + `agent_macros` (8+ tables); RLS isolates user-side ticket views from agent-side
- [x] **HELP-02**: In-app widget on every screen (KB search first → ticket form fallback if no helpful article)
...
- [x] **HELP-04**: AI assist via Claude (draft replies + auto-tag tickets + auto-route by topic) — agent reviews + edits + sends
- [x] **HELP-05**: CSAT survey auto-sent after ticket close via Resend (or SES if PHI-touching); 1-tap reply
- [x] **HELP-06**: SLA tracking with breach alerts; `pg_cron` checks open tickets against priority SLA, emits agent + on-call notifications
- [x] **HELP-07**: KB articles use markdown + react-markdown rendering + dompurify XSS protection; admin-editable with versioning
- [x] **HELP-08**: KB articles localized to Spanish (parallel `{slug}.es.md` files); locale picker per article
- [x] **HELP-09**: Realtime updates for agents + users via Supabase Realtime (typing indicator + live message arrival); reuses Phase 9 patterns
- [x] **HELP-10**: Macros / canned responses per-agent + per-team; agents insert via `/macro` slash command in reply composer
- [x] **HELP-11**: KB full-text search using Postgres `tsvector` + GIN indexes (English + Spanish dictionaries); upgrade to Typesense/Meilisearch ONLY if needed at v1.5+
- [x] **HELP-12**: Admin routing rules (tag → agent assignment); admin macro editor; admin SLA-targets editor; admin sentiment-alert thresholds
- [x] **HELP-13**: Admin sees per-tag-cluster ticket-volume trending dashboard (helps identify product-issue clusters before they spike)
...
- [x] **RECOMMEND-02**: OpenAI `text-embedding-3-small` routed via Vercel AI Gateway (same proxy posture as v1.2 Anthropic); embeddings only, not chat
- [x] **RECOMMEND-03**: Nightly cron embeds new content (community posts when M4 ships, KB articles, blog posts, course lessons) into `content_embeddings`
...
- [x] **RECOMMEND-05**: Weekly Claude summary email (short narrative + 1-3 suggested actions) sent via Resend (or SES if PHI); cron at 09:00 user-timezone Sunday
...
- [x] **RECOMMEND-09**: Pricing/offer personalization (annual nudge for monthly subscribers showing churn signal; discount eligibility per user's plan-history); plan-personalization Edge Fn called by PAYWALL + SAVE
...
- [x] **POLISH-05**: Smart notifications (email + web-push + in-app) with frequency-capping + snoozable + sentiment-aware per `notification_settings`
- [x] **POLISH-06**: User manages notification preferences in self-serve center (per-category opt-out: dose-reminders / AI-insights / clinic-alerts / billing / marketing)
- [x] **POLISH-07**: PWA + offline mode via `vite-plugin-pwa`; native-feeling install prompt on supported browsers
- [x] **POLISH-08**: Dark mode parity across all v1.3 new surfaces (admin shell + helpdesk + onboarding builder + clinic dashboard)
- [x] **POLISH-09**: WCAG 2.2 AA accessibility audit via axe-core in CI; keyboard nav + screen-reader labels + contrast + focus rings verified
- [x] **POLISH-10**: Public status page at `status.leanshot.app` via Better Stack ($12/mo); auto-incident detection from Sentry + Vercel + Supabase
- [x] **POLISH-11**: "What's New" in-app drawer surfaces shipped improvements (changelog-style, per-user dismissal state)
- [x] **POLISH-12**: Quarterly NPS survey (one-question + open-text follow-up) sent to active users; segmented by tenure + plan + cohort; results visible in admin

### WS19 — M4: Membership Tiers Extension (MEMBER, 4 REQ-IDs)

- [x] **MEMBER-01**: Lifetime tier added to `tier_effective` view + Stripe one-time price wiring; one-time payment grants permanent Pro entitlement
- [x] **MEMBER-02**: Grandfathering — admin sets per-cohort grandfathered pricing (e.g., "Pro early-adopters keep $9.99/mo when public price moves to $14.99")
- [x] **MEMBER-03**: Coupon-driven Pro upgrades + 7-day-trial extension for cancellation save flow (compounds with POLISH-04 + SAVE flow); admin-creatable coupons via Stripe Coupons + Promotion Codes
- [x] **MEMBER-04**: Community-gated content entitlements — Pro-only spaces / courses / events enforced via `tier_effective` lookup at COMMUNITY/COURSE/EVENT surfaces
...
- [x] **EVENT-04**: Automatic reminder emails (1 day before + 1 hour before) via Resend (or SES for PHI-touching clinic events); per-user-timezone-aware
- [x] **EVENT-05**: Post-event recording uploaded to Mux → attached as new lesson in adjacent course (optional, admin-toggled)
...
- [x] **TRAFFIC-01**: UTM source/medium/campaign/term/content captured verbatim on every visit; first-touch persisted ONCE per `lt_anon_id`, last-touch updated on every visit (D-01, D-02, D-13)
- [x] **TRAFFIC-02**: `lt_anon_id` HttpOnly Secure SameSite=Lax cookie (UUIDv4, Max-Age 7776000 = 90d sliding window) set server-side BEFORE the Vite SPA boots (Vercel Edge Middleware) (D-04)
- [x] **TRAFFIC-03**: PostHog `aliasServerSide(supabase.uid, lt_anon_id)` called on signup stitches anonymous distinct_id → identified user; `merge-anon-session` Edge Fn extended (D-04)
- [x] **TRAFFIC-04**: Operator-editable `channel_groups` taxonomy table with `match_rule_jsonb` rule matcher, priority order, attribution_window_days; 8 seed defaults; takes effect at next matview refresh — no deploy (D-01, D-07)
- [x] **TRAFFIC-05**: Operator-editable `referrer_channel_rules` table seeded with ~80 well-known referrer domains (Snowplow `referer-parser` JSON source); referrer-domain classification when UTM absent (D-03)
- [x] **TRAFFIC-06**: Three parallel conversion funnels — Consumer (visit→signup→activation→paid), Clinic-org (visit→clinic-signup→first-patient-added→first-paid-seat), Affiliate (visit→affiliate-signup→first-referral-conversion); audience switcher in Funnels tab (D-05)
- [x] **TRAFFIC-07**: Activation event (P34 ONBOARD) is north-star for ad-spend join; `traffic_channel_rollup` LEFT JOINs `ad_spend_facts` (P33) at `(channel_group, day)` grain with fixed network→channel_group mapping; surfaces `cac_to_activation` (primary) + `cac_to_paid` (secondary) (D-06, D-15)
- [x] **TRAFFIC-08**: Family of 4 surfaces — 3 matviews (`traffic_channel_rollup`, `traffic_funnel_rollup`, `traffic_landing_page_rollup`) + 1 regular VIEW (`traffic_realtime_v`) refreshed CONCURRENTLY with UNIQUE indexes; refresh sequenced AFTER P33's existing `ad_revenue_refresh` cron in a single pg_cron job (D-11, D-14)
- [x] **TRAFFIC-09**: Per-channel cohort retention curves D1/D7/D14/D30/D60 on `traffic_channel_rollup` via SECDEF helper `is_retained(user_id, audience, window_days)` with per-audience semantics (D-16)
- [x] **TRAFFIC-10**: Per-clinic-org dashboard scope — `org_id` dimension on `user_traffic_attribution` + all 4 matviews; SECDEF accessors gate on `is_admin()` OR `_is_org_clinician(org_id)`; cross-tenant impersonation RLS deny test required (D-12)
- [x] **TRAFFIC-11**: `funnel-anomaly-cron` extension scans EACH (channel_group × audience × funnel_stage_pair × date) for per-channel-stage drops vs 7-day baseline; writes to existing `admin_notifications` surface; dedup key = `(channel_group, audience, funnel, stage_pair, date)` — no new cron (D-08)
- [x] **TRAFFIC-12**: `growth/traffic` admin module (sibling of `growth/cac` under shared 'Growth' nav group) renders 4 tabs (Channels / Funnels / Landing Pages / Real-time) + Taxonomy admin sub-page; Pill PillGroup tab control; native `useEffect + setInterval` polling on Real-time (no TanStack Query); 4 typography sizes (11/13/18/28) / 2 weights (D-09, D-10, D-11, D-12)
...
| ADMIN-06 | Phase 27 | Complete |
| TAXO-01 | Phase 24 | Complete |
| TAXO-02 | Phase 24 | Complete |
| TAXO-03 | Phase 27 | Complete |
| TAXO-04 | Phase 24 | Complete |
| TAXO-05 | Phase 27 | Complete |
| TAXO-06 | Phase 24 | Complete |
| AFFTIER-01 | Phase 26 | Complete |
| AFFTIER-02 | Phase 26 | Complete |
| AFFTIER-03 | Phase 26 | Complete |
| AFFTIER-04 | Phase 26 | Complete |
| AFFTIER-05 | Phase 26 | Complete |
| AFFTIER-06 | Phase 26 | Complete |
...
| EMBED-01 | Phase 41 | Complete |
| EMBED-02 | Phase 41 | Complete |
| EMBED-03 | Phase 41 | Complete |
| EMBED-04 | Phase 41 | Complete |
| EMBED-05 | Phase 41 | Complete |
| EMBED-06 | Phase 41 | Complete |
| EMBED-07 | Phase 41 | Complete |
| EMBED-08 | Phase 41 | Complete |
...
| I18N-01 | Phase 32 | Complete |
| I18N-02 | Phase 32 | Complete |
| I18N-03 | Phase 32 | Complete |
| I18N-04 | Phase 32 | Complete |
| I18N-05 | Phase 32 | Complete |
| I18N-06 | Phase 32 | Complete |
| I18N-07 | Phase 32 | Complete |
| I18N-08 | Phase 32 | Complete |
| I18N-09 | Phase 32 | Complete |
| I18N-10 | Phase 32 | Complete |
| ORG-01 | Phase 28 | Complete |
| ORG-02 | Phase 28 | Complete |
| ORG-03 | Phase 28 | Complete |
| ORG-04 | Phase 28 | Complete |
| ORG-05 | Phase 28 | Complete |
| ORG-06 | Phase 28 | Complete |
...
| CLIN-01 | Phase 30 | Complete |
| CLIN-02 | Phase 30 | Complete |
| CLIN-03 | Phase 30 | Complete |
| CLIN-04 | Phase 30 | Complete |
| CLIN-05 | Phase 30 | Complete |
| CLIN-06 | Phase 30 | Complete |
| CLIN-07 | Phase 30 | Complete |
| CLIN-08 | Phase 30 | Complete |
...
| ONBOARD-02 | Phase 34 | Complete |
| ONBOARD-03 | Phase 34 | Complete |
| ONBOARD-04 | Phase 34 | Complete |
| ONBOARD-05 | Phase 34 | Complete |
| ONBOARD-06 | Phase 34 | Complete |
| ONBOARD-07 | Phase 34 | Complete |
| ONBOARD-08 | Phase 34 | Complete |
| ONBOARD-09 | Phase 34 | Complete |
| ONBOARD-10 | Phase 34 | Complete |
| ONBOARD-11 | Phase 34 | Complete |
| ONBOARD-12 | Phase 34 | Complete |
| ONBOARD-13 | Phase 34 | Complete |
| GAME-01 | Phase 35 | Complete |
| GAME-02 | Phase 35 | Complete |
| GAME-03 | Phase 35 | Complete |
| GAME-04 | Phase 35 | Complete |
| GAME-05 | Phase 35 | Complete |
| GAME-06 | Phase 35 | Complete |
| GAME-07 | Phase 35 | Complete |
| GAME-08 | Phase 35 | Complete |
| GAME-09 | Phase 35 | Complete |
| REVIEW-01 | Phase 36 | Complete |
| REVIEW-02 | Phase 36 | Complete |
| REVIEW-03 | Phase 36 | Complete |
| REVIEW-04 | Phase 36 | Complete |
| REVIEW-05 | Phase 36 | Complete |
| REVIEW-06 | Phase 36 | Complete |
| REVIEW-07 | Phase 36 | Complete |
| REVIEW-08 | Phase 36 | Complete |
| HELP-01 | Phase 37 | Complete |
| HELP-02 | Phase 37 | Complete |
| HELP-03 | Phase 37 | Complete |
| HELP-04 | Phase 37 | Complete |
| HELP-05 | Phase 37 | Complete |
| HELP-06 | Phase 37 | Complete |
| HELP-07 | Phase 37 | Complete |
| HELP-08 | Phase 37 | Complete |
| HELP-09 | Phase 37 | Complete |
| HELP-10 | Phase 37 | Complete |
| HELP-11 | Phase 37 | Complete |
| HELP-12 | Phase 37 | Complete |
| HELP-13 | Phase 37 | Complete |
| RECOMMEND-01 | Phase 38 | Complete |
| RECOMMEND-02 | Phase 38 | Complete |
| RECOMMEND-03 | Phase 38 | Complete |
| RECOMMEND-04 | Phase 38 | Complete |
| RECOMMEND-05 | Phase 38 | Complete |
| RECOMMEND-06 | Phase 38 | Complete |
| RECOMMEND-07 | Phase 38 | Complete |
| RECOMMEND-08 | Phase 38 | Complete |
| RECOMMEND-09 | Phase 38 | Complete |
...
| POLISH-05 | Phase 42 | Complete |
| POLISH-06 | Phase 42 | Complete |
| POLISH-07 | Phase 42 | Complete |
| POLISH-08 | Phase 42 | Complete |
| POLISH-09 | Phase 42 | Complete |
| POLISH-10 | Phase 41 | Complete |
| POLISH-11 | Phase 42 | Complete |
| POLISH-12 | Phase 42 | Complete |
| MEMBER-01 | Phase 43 | Complete |
| MEMBER-02 | Phase 43 | Complete |
| MEMBER-03 | Phase 43 | Complete |
| MEMBER-04 | Phase 43 | Complete |
...
| EVENT-04 | Phase 47 | Complete |
| EVENT-05 | Phase 47 | Complete |

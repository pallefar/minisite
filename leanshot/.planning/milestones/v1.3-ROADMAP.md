...
requirements_archive: .planning/milestones/v1.3-REQUIREMENTS.md
uat_deferred_runbook: .planning/v1.3-uat-deferred.md
---

# Milestone v1.3: Platform Expansion — Revenue + Depth + B2B + HIPAA + M4 Community

**Status:** SHIPPED 2026-05-25
**Phases:** 24-51 (28 phases)
**Total plans:** 242 plan files across 28 phase directories (`-PLAN.md`)
**Audit:** tech_debt — 206/206 REQ-IDs functionally satisfied (`v1.3-MILESTONE-AUDIT.md`)
**Timeline:** 2026-05-17 → 2026-05-25 (8 days of intensive parallel-execution; multi-month brief compressed via aggressive SDK/CARL orchestration)
**Git range:** `840adbdc..ffa86396` — 1,430 commits since v1.2 ship

## Overview

Triple-bet release that lands paying clinics under BAA, deepens consumer product on web, tightens unit economics, and ships M4 Membership/Community in-house. Foundation layer (modular admin + canonical event taxonomy + server-side PostHog + HIPAA engineering controls) underpins three strategic bets:

1. **A — Revenue / Growth** — multi-tier affiliate, mid-trial paywall A/B, page-builder A/B, hourly ad-spend ETL across Meta/Google/TikTok with CAC dashboard.
2. **B — Product depth (web)** — embed-provider blocks (Calendly/YouTube/Tally with consent + CSP), pharmacology paywall with safety-info carve-out, Spanish i18n runtime.
3. **C — B2B clinic + HIPAA** — `organizations` schema + 4-leak-vector RLS, per-patient metered billing, white-label theming, clinician dashboard + dose-trend alerts, full BAA chain across 6 critical vendors.

Plus **User-facing depth + retention** (M2 onboarding overhaul with activation event, M3 gamification + review prompt engine, M6 helpdesk core, M5b AI recommender with pgvector + Claude digest, save-offers cancellation flow, public status page) and **M4 Membership/Community Platform** (in-house Skool-style: feed + spaces + DMs + courses + events + moderation + search/digests).

Mobile (P16-21) deliberately deferred to v1.4 per Path-1 megamilestone framing.

## Phases shipped

- [x] Phase 24: Foundation — Modular Admin Shell + Event Taxonomy + Server-side PostHog (complete)
- [x] Phase 25: HIPAA Audit Hardening + Vendor BAA Chain (10/10 plans, shipped 2026-05-18)
- [x] Phase 26: Multi-Tier Affiliate — Standard / Gold / Lifetime (complete)
- [x] Phase 27: Modular Admin Shell Extensions (complete; nyquist compliant)
- [x] Phase 28: Clinic Organizations — Schema + RLS Hardening (8 plans; complete)
- [x] Phase 29: Org Subscriptions + Per-Patient Metered Billing (8 plans; complete; HUMAN-VERIFY)
- [x] Phase 30: Clinician Dashboard + Custom Rank Weights + Dose-Trend Alerts (complete; HUMAN-VERIFY)
- [x] Phase 31: White-Label + Org Roles + Clinic Onboarding Builder (8/8; shipped 2026-05-18)
- [~] Phase 32: Spanish i18n — **PARTIAL**, 6/7 plans; 32-06 ES contractor delivery deferred to v1.4
- [x] Phase 33: Hourly Ad-Spend ETL (Meta/Google/TikTok) (5/5; shipped 2026-05-18)
- [~] Phase 34: M2 Onboarding Overhaul + Activation Event — **PARTIAL**, 9/10 plans; 34-08 walkthrough + 34-10 Apple/PostHog checkpoints deferred to v1.4 (ONBOARD-02 magic-link + Google live)
- [x] Phase 35: M3 Gamification Engine (complete + approved automated-verify-only; 6 HUMAN-UAT)
- [x] Phase 36: M3 Review Prompt Engine — Web Only (5 plans; complete; 3 HUMAN-UAT)
- [x] Phase 37: M6 Helpdesk Core (9 plans; complete)
- [x] Phase 38: M5b AI Recommender (pgvector + Claude Digest) (10/10 plans complete)
- [x] Phase 39: A/B Trifecta — Mid-Trial Paywall + Pharma Paywall + Page-Variant A/B (10 plans, 6 waves; automated-verify-only; 8 HUMAN-UAT)
- [x] Phase 40: Cancellation Save-Offers Flow (6 plans; automated-verify-only; 4 HUMAN-UAT)
- [x] Phase 41: Public Status Page + Embed-Provider Blocks (6 plans, 3 waves; automated-verify-only; 6 HUMAN-UAT + CR-02)
- [x] Phase 42: v1.3 Polish Closeout — WCAG 2.2 AA + Notifications + PWA + Dark Mode + What's New + NPS (complete; 5 device-UAT signals → v1.4)
- [x] Phase 43: M4 Membership Tiers Extension — Lifetime + grandfathering (6 plans; complete; 4 HUMAN-UAT)
- [x] Phase 44: M4 Community Feed Foundation (10 plans; complete; 4 HUMAN-UAT)
- [x] Phase 45: M4 Community Spaces + Member Directory + Opt-in DMs + Leaderboard (9 plans, 4 waves; complete)
- [x] Phase 46: M4 Courses / Classroom — Mux video + certificates (11 plans, 4 waves; complete)
- [x] Phase 47: M4 Events Calendar + Zoom + Reminders + Recording (12 plans; complete)
- [x] Phase 48: M4 Moderation — reports + bans + banned-words + Claude auto-flag (12 plans; complete)
- [x] Phase 49: M4 Search + Email Digests — Postgres FTS + Resend (complete)
- [~] Phase 50: Admin-Curated RAG Knowledge Base — **PARTIAL**, Wave 1 only (4/9 plans). Waves 2-4 (50-05..09: scrape/summarize/queue/embeddings/AI-coach citations/Tip-of-day/newsletter/public hub) deferred to v1.4 per D-22 STRETCH cut.
- [x] Phase 51: Full Traffic + Conversion Tracking + UTM Attribution + Unified Dashboard (10 plans, 5 waves; automated-verify-only; 6 HUMAN-UAT)

## Key accomplishments

1. **HIPAA + clinic infra is live in code** — Phase 25 ships every engineering control (BAA-scope guard in `ai-chat`, Stripe PHI lint, vendor BAA chain table + subprocessor cron, `_shared/email-router.ts` PHI Resend↔SES split, Sentry mask audit, PostHog session-replay PHI guard, clinician MFA hard-cut). Phases 28-31 land the multi-tenant `organizations` schema with 4-leak-vector RLS, `withOrgScope` compile-time-enforced wrapper, HMAC realtime channels, JWT `org_ids` propagation, JIT clinic-org onboarding under separate Stripe namespace + per-patient metered billing, and per-clinic ranking weights + nightly dose-trend alerts with ack/snooze.

2. **A/B + paywall + save-offers form a closed-loop revenue engine** — Phase 39 ships the A/B trifecta (mid-trial paywall + page-variant + pharma paywall) with composite-goal Bayesian decisioning, 42-day variant auto-archive, refund-rate kill-switch with Slack alerts, canonical-link discipline, and the 3-layer phaCheck() safety-info carve-out (ESLint AST rule + runtime helper + CI grep gate). Phase 40 wires the cancellation save-offers flow (pause/downgrade/discount/extended-trial) with admin ROI dashboard. Phase 43 lands the Lifetime membership tier with grandfathering coupons and `tier_effective`-based community/course/event entitlements.

3. **M2/M3/M5b/M6 retention surface** — Phase 34 lands the new front-door funnel: anonymous-session preview → magic-link/Google OAuth → activation event locked as TAXO-02 server-side signal consumed by Phase 36 NPS + Phase 38 recommender + Phase 39 paywall. Phase 35 ships the gamification engine (XP ledger + streaks + freeze tokens + cohort-scoped leaderboard matview + OG share cards via Vercel Function). Phase 37 lands the helpdesk core with Resend Inbound webhook + HMAC reply-threading + Claude AI-assist. Phase 38 lands pgvector + HNSW + OpenAI embeddings + nightly weekly-Claude-digest with red-flag guardrail and HITL review queue.

4. **M4 Membership/Community ships in-house on Supabase + Mux** — Phase 43-49 deliver the full Skool-equivalent stack: tier-gated community feed (posts/comments/reactions/@mentions/Mux video + Realtime), spaces + member directory + opt-in DMs + community leaderboard, courses (modules + lessons + Mux signed playback + jsPDF certificates + public `/verify/<cert_id>?t=<hmac>` route), events (capacity + waitlist + Zoom S2S OAuth + timezone-aware reminders + Mux recording → lesson), moderation (reports queue + mute/ban/temp-suspend + banned-words sweep + Claude auto-flag + immutable audit log + session DELETE for ban-enforcement), and Postgres FTS search + daily/weekly Resend digests.

5. **Embeds + status page + traffic analytics close the operator surface** — Phase 41 ships embed-provider blocks (Calendly OAuth + YouTube-nocookie + Tally + custom-iframe) gated by Vercel Edge Middleware dynamic CSP allowlist with consent-gating, plus a Better Stack status page. Phase 51 lands the full multi-channel traffic + conversion tracking system: `user_traffic_attribution` first/last-touch, `lt_anon_id` HttpOnly cookie minted by Edge Middleware, PostHog server-side alias on signup, 3 funnel audiences (consumer / clinic / affiliate) with D1/D7/D14/D30/D60 retention curves, CAC-to-activation join against Phase 33 `ad_spend_facts`, and the unified `growth/traffic` admin module (Channels / Funnels / Landing Pages / Real-time).

6. **Affiliate v2 + ad-spend ETL deepen unit-economics** — Phase 26 extends the v1.2 affiliate program with stamped-at-conversion-time tier accounting (Standard → Gold auto / Gold → Lifetime admin grant), monthly Lifetime recurring-commission cron, ratio Z-score anomaly detector, and tier-aware co-branded landing pages. Phase 33 lands 3 hourly Edge Fns (Meta + Google + TikTok) populating `ad_spend_facts` (partitioned by month) with idempotent 72h re-sync, daily gap-detection cron, ECB-driven `fx_rates`, and a `ad_revenue_normalized` view powering the admin CAC dashboard with 7-day rolling LTV-ratio alerts.
...
**Key decisions (per audit + per-phase CONTEXT.md decision logs):**

- **Path 1 megamilestone over hybrid carve / sequential / tight-v1.3** (2026-05-17): accept all 18 workstreams + every differentiator at the cost of v1.4 mobile slipping a quarter.
- **In-house M4 Community on Supabase + Mux** over Skool integration or 3rd-party — 6-8 phases added but full control over tier-gating, RLS, and Realtime.
- **Server-side PostHog via `_shared/posthog-server.ts`** (Phase 24 D-13) — every Edge Fn awaits `ph.shutdown()` before return. Eliminates adblocker corruption for signup/payment/activation/refund.
- **Dual Anthropic credentials** (consumer Moonshot via OpenRouter vs clinical Anthropic with BAA-scope guard) — `ai-chat` branches on `org_id IS NOT NULL` (Phase 25 + 37 + 38 + 48).
- **3-layer phaCheck() enforcement** — ESLint AST rule + runtime helper + CI grep gate (Phase 39 D-06 / PHARMA-02 safety-info carve-out). Validated as repeatable pattern in feedback_3_layer_must_never_invariant_pattern.
- **`tier_effective.has_active` canonical entitlement** (NOT `tier='paid'` string match) — reused across Phase 29 + 39 + 43 + 44 + 46.
- **i18n routing = `?lang=es` query** (NOT `/es/` path prefix) — avoids Vercel rewrite doubling.
- **Resend↔SES PHI router** (Phase 25-03) — `phi:boolean` switch on template metadata; consumer paths stay on Resend, clinical paths route via SES under BAA.
- **`withOrgScope` 4-layer multi-tenant defense** (Phase 28-02) — brand types + ESLint rule + Proxy runtime + Sentry fatal.

**Issues resolved during milestone:**

- HIPAA BAA chain signed across 6 critical vendors (Supabase / Vercel / Sentry / Anthropic / AWS SES / PostHog) — first clinic deal unblocked.
- Mux pair-deploy passthrough drift (44↔46↔47) — documented in carry-overs; deployed atomically.
- vite-plugin-pwa strategy: `injectManifest` + custom `src/sw.ts` (generateSW does not support push/notificationclick).
- Postgres back-dated migration ordering issue documented in reference_supabase_back_dated_migration_blocks_push.
- ROADMAP plan-checkbox drift mitigated for 39/40/44 (per-plan toggles maintained); 50+ stale rows persist for earlier phases (cosmetic).

**Issues deferred to v1.4:**

- Phase 32-06 Spanish contractor delivery (bilingual clinical) — I18N-04/05/06/09 partial.
- Phase 34 Apple OAuth wiring (ONBOARD-02) + 34-08 admin walkthrough + 34-10 PostHog Personal API key checkpoint.
- Phase 50 Waves 2-4 (RAG scrape/summarize/queue/embeddings/AI-coach citations/Tip-of-day/newsletter/public hub) — D-22 explicitly STRETCH cut.
- Phase 42 5 device-UAT signals (axe-core CI baseline, push device smoke, dark-mode VR snapshots, PWA installability).
- v1.2 carry-over: Phases 16-21 (Capacitor mobile shells, push, HealthKit, ad network, watch apps).
- 33 consolidated HUMAN-UAT operator runbook signals across 7 phases (`v1.3-uat-deferred.md`).
- 7 vendor-secret deferrals (Better Stack, Calendly OAuth, Sentry CSP, traffic-recorder env, SHARE_TOKEN, NPS signing key, PostHog Personal API key, Slack webhook).

**Technical debt incurred (per audit):**

- REQUIREMENTS.md checkbox drift — 102/206 `[ ]` despite owner-phase complete (cosmetic; same pattern recurs from v1.1; bulk-toggled to `[x]` in this archive).
- ROADMAP.md plan-checkbox drift — 50+ `- [ ] NN-PP-PLAN.md` rows for shipped plans. Phase 39/40/44 maintained; 26/28/30/35 did not (per feedback_roadmap_format_variance_close_out_check).
- SUMMARY frontmatter `requirements:` array adoption uneven — 38/200+ populated. Older phases (24-31) rarely tag; newer phases (43-49) consistent.
- VALIDATION.md `status: draft`/`planned` flag never flipped post-merge on 10 phases (24-26-28-29-30-31-36-47-49 + 33 research-derived). Same v1.1 pattern at higher volume.
- Phase 41 CR-02 — Calendly OAuth popup nav lacks signed handoff token contract (Edge Fn `verify_jwt = false` but upstream popup launch unguarded). Single-plan ≤30min fix.

**Stats:**

- 28 phases / 242 plan files / 237 plan SUMMARYs on disk
- 1,430 git commits since v1.2 ship (2026-05-17 → 2026-05-25)
- 329,583 LOC across `leanshot/src/` + `supabase/` (`.ts`/`.tsx`/`.sql`)
- 25 of 28 phases at `disk_status=complete`; 3 partial (32 vendor-deferred, 34 fixture-deferred, 50 deferred to v1.4)
- 13/27 phases `nyquist_compliant: true`; 8 `draft` (work shipped, paperwork lagged); 4 missing VALIDATION.md
- 206 REQ-IDs across 25 workstreams; 206/206 functionally satisfied; 102/206 REQUIREMENTS.md `[x]` toggled in this archive

---

_For current project status, see `.planning/ROADMAP.md`. For the consolidated UAT runbook, see `.planning/v1.3-uat-deferred.md`. For the audit, see `v1.3-MILESTONE-AUDIT.md`._

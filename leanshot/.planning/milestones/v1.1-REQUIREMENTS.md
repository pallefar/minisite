# Requirements: LeanShot — Milestone v1.1 (ARCHIVED)

**Defined:** 2026-05-10
**Shipped:** 2026-05-13
**Final commit:** `b24f26d` on `origin/main`
**Audit:** `leanshot/.planning/v1.1-MILESTONE-AUDIT.md` — verdict `tech_debt` (no functional blockers; 48/49 fully satisfied; 1/49 partial — CLINIC-07 operator-side per-patient drill-in "View activity" button is a `console.warn` stub; data reachable via org-wide AuditTab. WARNING, not BLOCKER. Carried to v1.2.)
...
- [x] **COMPL-04**: App displays prominent "Not medical advice — consult your healthcare provider" disclaimer on first run and overlaid on the drug-level chart, calibrated for FDA general-wellness software guidance
- [x] **COMPL-05**: Marketing copy and in-app framing avoid mental-health treatment claims (Mood tab + AI coach) to keep the app outside CMIA's mental-health digital service definition
...
- [x] **PROD-01**: App is deployed at a real custom domain over HTTPS — static SPA hosted on Vercel/Cloudflare Pages/Netlify (decision deferred to deploy phase)
- [x] **PROD-02**: Real-user JS errors are captured by Sentry with PII redaction configured for symptom/mood/AI fields
- [x] **PROD-03**: Privacy-respectful product analytics (PostHog cookieless mode) measure feature usage, onboarding drop-off, and core funnels — without leaking health content
- [x] **PROD-04**: A test runner (Vitest 4 + React Testing Library + Playwright) is configured with `npm test` running in CI on every PR
- [x] **PROD-05**: ESLint + Prettier + a typecheck step run in CI; the existing `tsc -b --noEmit` is wired to PR checks
- [x] **PROD-06**: Marketing landing page (`Landing.tsx`) is hosted on a separate subdomain or path so health-app analytics and consents stay clean
...
- [x] **AUTH-01**: User can create an account with email + password using Supabase Auth
- [x] **AUTH-02**: User receives an email verification on signup and cannot fully use the app until verified (Supabase confirmation flow)
- [x] **AUTH-03**: User can sign in across devices and the session persists across browser refresh (Supabase session in localStorage with refresh-token rotation)
- [x] **AUTH-04**: User can reset their password via emailed link (Supabase password reset)
- [x] **AUTH-05**: User can sign out, and signing out clears local sensitive caches (sync queue, AI history)
- [x] **AUTH-06**: User can continue to use the app fully offline once signed in (auth required only for cloud sync, not for local logging)
...
- [x] **SYNC-01**: A signed-in user's tracked data (injections, weights, photos, meals, supplements, mood, sleep, symptoms, settings) syncs across their devices via Supabase
- [x] **SYNC-02**: An existing local-only `leanshot_v4` user can sign in and have their localStorage data uploaded into their account on first sync, with no data loss
- [x] **SYNC-03**: The pre-cloud `leanshot_v4` snapshot is preserved as `leanshot_v4_pre_cloud_backup` for at least 90 days post-migration so users can recover if migration fails (mitigates the lossy v3→v4 pattern flagged in CONCERNS.md)
- [x] **SYNC-04**: Mutations made offline are queued in IndexedDB and replayed on reconnect; conflicts resolve last-writer-wins with a clear UI when collisions occur
- [x] **SYNC-05**: All Supabase tables enforce per-user scoping via Row-Level Security policies (`auth.uid() = user_id`, default-deny) — RLS is the primary tenant-isolation primitive, not application-layer filtering
- [x] **SYNC-06**: Photos move from base64-in-Zustand to Supabase Storage with signed URLs, keeping the Zustand-persisted slice lean (current photos slice is the largest contributor to localStorage size)
...
- [x] **PK-01**: `src/lib/pharmacology.ts` (`calcMedLevel`, `HALF_LIVES`, `TITRATION`) is covered by an automated test corpus citing peer-reviewed sources (Schneck 2024 for tirzepatide PK; FDA clinical pharmacology reviews for semaglutide/liraglutide)
- [x] **PK-02**: `src/lib/insights.ts` (`generateInsights`, `pickFocus`) is covered by automated tests including a refusal-list for any insight that could read as dose-change advice
- [x] **PK-03**: The drug-level chart visually conveys uncertainty (shaded inter-individual variability band) rather than a single deterministic line
- [x] **PK-04**: The chart and any printed/shared report carry an explicit "estimate, not measured serum level — based on population pharmacokinetics" disclaimer
- [x] **PK-05**: The pharmacology engine version is recorded in saved data so a future v1.1 two-compartment upgrade can be applied retroactively without ambiguity
...
- [x] **SHARE-01**: Patient can create a share link from Settings that grants a doctor read-only in-browser access to the patient's data, scoped to a time window the patient picks
- [x] **SHARE-02**: Doctor opens the share link, enters a 6-digit access code (delivered out-of-band), and sees a read-only view of the patient's same data — including live drug-level chart, recent injections, symptoms, photos, weight, doctor report
- [x] **SHARE-03**: AI conversation history is NEVER included in the doctor view (privacy guarantee)
- [x] **SHARE-04**: Patient can revoke a share at any time and the doctor's open page becomes unusable within seconds — verified by an automated 4-failure-mode revocation drill (token cache, HTTP cache, JWT TTL, forwarded link)
- [x] **SHARE-05**: All share-link reads are audit-logged and visible to the patient in Settings (which doctor, when, what)
- [x] **SHARE-06**: Doctor view delivers `Cache-Control: private, no-store` and is bound to a recipient identifier so a forwarded link to a different device fails
...
- [x] **CLINIC-04**: Clinic operator sees a roster of all linked patients with at-a-glance status (recent dose, active streak, recent symptoms, missed-dose flag) — powered by running `pickFocus`/`generateInsights` per-patient as `rankPatients(orgState)`
- [x] **CLINIC-05**: Clinic operator can drill into any one patient and see the same read-only view used by SHARE-02 (component reuse)
...
- [x] **CLINIC-07**: All clinic operator actions are audit-logged (which operator viewed which patient, when), surfaced to both patient and org owner — capture half shipped Phase 9 (audit_logs writes via SECURITY DEFINER RPCs + log_clinic_event); operator/patient-facing audit surface UI shipped Phase 10

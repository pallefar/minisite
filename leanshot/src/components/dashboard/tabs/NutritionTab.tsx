    ...
    <div className="grid grid-cols-12 gap-4 md:gap-5 stagger">
      <Card span={12}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Macro label="Protein" value={protein} target={u.proteinTarget} unit="g" color="var(--color-success)" />
          <Macro label="Calories" value={calories} target={u.calorieTarget} unit="kcal" color="var(--color-primary)" />
          <Macro label="Fiber" value={fiber} target={u.fiberTarget} unit="g" color="var(--color-amber)" />
          <Macro label="Water" value={waterToday} target={u.waterTarget} unit="cups" color="#5AB7C7" />
        </div>
      </Card>

      <Card span={7}>
        <CardHeader title="Quick log meal" icon={<Sparkles className="size-4" />} action={<Badge tone="info">AI</Badge>} />
        <div className="space-y-3">
          ...
          />
          <Button variant="ghost" block leadingIcon={<Sparkles className="size-4" />} loading={aiBusy} onClick={aiEstimate}>
            {aiBusy ? 'Estimating…' : 'Estimate macros with AI'}
          </Button>
          <div className="grid grid-cols-2 gap-3">
            <Input label="Calories" type="number" inputMode="numeric" value={meal.cal} onChange={(e) => setMeal({ ...meal, cal: e.target.value })} />
            <Input label="Protein (g)" type="number" inputMode="numeric" value={meal.pro} onChange={(e) => setMeal({ ...meal, pro: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input label="Fiber (g)" type="number" inputMode="numeric" value={meal.fib} onChange={(e) => setMeal({ ...meal, fib: e.target.value })} />
            <Input label="Hunger (1–10)" type="number" min={1} max={10} inputMode="numeric" value={meal.hunger} onChange={(e) => setMeal({ ...meal, hunger: e.target.value })} />
          </div>
          <Input label="Satisfaction after" type="number" min={1} max={10} inputMode="numeric" value={meal.sat} onChange={(e) => setMeal({ ...meal, sat: e.target.value })} />
          <Button block onClick={submit}>Log meal</Button>
        </div>
      </Card>

      <Card span={5}>
        <CardHeader title="Water & food noise" icon={<Droplet className="size-4" />} />
        <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--color-text-secondary)] mb-2">Water (cups, 8oz)</p>
        <div className="flex flex-wrap gap-1.5 mb-4">
          {Array.from({ length: Math.max(u.waterTarget, waterToday) + 2 }, (_, i) => i + 1).map((n) => (
            <button
              key={n}
        ...
        <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--color-text-secondary)] mb-2">Food noise (1=silent, 10=loud)</p>
        ...
        <p className="text-[11px] text-[var(--color-text-tertiary)] mt-2">Lower = the medication is working.</p>
      </Card>

      <Card span={12}>
        <CardHeader title="Today's meals" icon={<ListChecks className="size-4" />} />
        {todayMeals.length === 0 ? (
          <p className="text-[13px] text-[var(--color-text-tertiary)] text-center py-4">No meals today.</p>
        ) : (
                ...
                {todayMeals.map((m) => {
                  ...
                  return (
                    <tr key={m.ts} className="border-t border-[var(--color-border)]">
                      ...
                      <td className="py-2 px-1 text-[var(--color-text-secondary)]">{m.hunger ?? '—'}→{m.satisfaction ?? '—'}</td>
                      <td className="py-2 px-1 text-right">
                        <button onClick={() => removeMeal(realIdx)} aria-label="Delete meal" className="size-7 rounded-md text-[var(--color-text-tertiary)] hover:bg-[var(--color-surface-elevated)] inline-flex items-center justify-center">
                          <X className="size-4" />
                        </button>
                      </td>
                    </tr>
...
function Macro({ label, value, target, unit, color }: { label: string; value: number; target: number; unit: string; color: string }) {
  const pct = target > 0 ? Math.min(100, (value / target) * 100) : 0;
  return (
    <div className="flex flex-col items-center text-center gap-1">
      <ProgressRing value={pct} size={84} strokeWidth={6} color={color}>
        <div>
          <p className="text-[15px] font-extrabold leading-tight numerals-tabular">{Math.round(value)}</p>
          <p className="text-[9px] text-[var(--color-text-tertiary)] font-semibold">/{target}{unit}</p>
        </div>
      </ProgressRing>

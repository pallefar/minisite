...
// ---------------------------------------------------------------------------
// Phase 56 Plan 56-06 (AD-11) — HealthKit firewall regression at the ad boundary.
//
// Proves the firewall still throws on health-shaped targeting objects now that
// ad-serving code (56-03 ads.ts) exists and calls assertNoHealthData at every
// ad-SDK boundary. This section documents INTENT: ads.ts must call
// assertNoHealthData before passing any data to ad-parameter construction.
//
// If these tests break, the Phase 55 health firewall has been weakened.
// ---------------------------------------------------------------------------

describe('Phase 56 AD-11 regression — firewall survives ad-serving code landing', () => {
  // Regression: the shape ads.ts passes to assertNoHealthData in a safe path
  // (ad-params with no health fields) must NOT throw.
  it('does NOT throw on a typical ad-param object (adUnitId + surface) — safe ad boundary', () => {
    expect(() => {
      assertNoHealthData(
        { adUnitId: 'ca-app-pub-test', surface: 'home', tier: 'free' },
        'buildAdParams',
      );
    }).not.toThrow();
  });

  it('does NOT throw on an ad impression payload (no health fields)', () => {
    expect(() => {
      assertNoHealthData(
        { placement: 'free-tier-banner', timestamp: 1716700000000, userId: 'u-abc' },
        'recordImpression',
      );
    }).not.toThrow();
  });

  // Regression: if any health-shaped field leaks into ad params, the firewall
  // MUST still throw even after 56-03 ad code is present.
  it('throws if a health-shaped field leaks into ad params ({ weight } regression)', () => {
    expect(() => {
      assertNoHealthData(
        { adUnitId: 'ca-app-pub-test', weight: 80 },
        'buildAdParams',
      );
    }).toThrow('Two-tunnel firewall violation');
  });

  it('throws if a health-shaped field leaks into ad params ({ steps } regression)', () => {
    expect(() => {
      assertNoHealthData(
        { surface: 'home', steps: 10000 },
        'buildAdParams',
      );
    }).toThrow('Two-tunnel firewall violation');
  });

  it('throws if a health-shaped field leaks into ad params ({ bodyMass } regression)', () => {
    expect(() => {
      assertNoHealthData(
        { adUnitId: 'ca-app-pub-test', bodyMass: 75 },
        'buildAdParams',
      );
    }).toThrow('Two-tunnel firewall violation');
  });

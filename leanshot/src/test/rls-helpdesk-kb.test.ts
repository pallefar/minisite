...
describeIfLive('Phase 37 Plan 37-09 — RLS impersonation: kb_articles / macros / routing / sla', () => {
  ...
  it('non-org-member CANNOT INSERT helpdesk_routing_rules', async () => {
    const { error } = await userA_patientClient
  ...
  it('search_kb_articles returns global rows for patient; cross-org search returns 0', async () => {
    ...
    const { data: crossRows, error: cErr } = await userA_patientClient.rpc(
      'search_kb_articles',
      { p_query: TEST_SLUG_PREFIX, p_locale: 'en', p_org_id: orgB, p_limit: 10 },
    );

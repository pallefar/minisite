-- Phase 30 Plan 00 — Task 2 (migration 5/5)
-- Extends rank_org_patients SECDEF to read org_settings.ranking_weights.
-- Per CONTEXT D-04; RESEARCH §Pattern 3.
--
-- EXTENSION POINT (NOT a replacement):
--   Reads org_settings.ranking_weights for p_org_id into v_weights.
--   If NULL (no custom weights): uses Phase 10 hardcoded defaults exactly.
--   If NOT NULL: multiplies signal scores by extracted weight scalars.
--
-- Phase 10 hardcoded baseline weights (from 20260901000003_rank_org_patients_rpc.sql):
--   missed_dose: 28 (flag × weight)
--   symptom_severity: 5 (per severity unit)
--   weight_trend_reversal: 15 (flag × weight)
--   days_since_injection: max 20 (linear cap)
--   streak_break: 12 (flag × weight)
--
-- RESEARCH Pitfall 3: never inject jsonb directly into EXECUTE format() — extract
-- into scalar variables and pass as %L literals.
--
-- RankRosterRow shape preserved exactly (returns same columns as Phase 10 baseline).
--

/**
 * index.test.ts — Vitest unit tests for rag-federated-dailymed handler.
 *
 * Phase 60 Plan 60-07. Task 4 (TDD RED → GREEN).
 *
 * 6 tests:
 *  T1: POST {topic_tags, mode:'incremental'} → searchSPLs → getSPLDetail → INSERT
 *  T2: enabled=false → 403
 *  T3: Dedup on content_hash (same setid:version → skipped_duplicate)
 *  T4: last_sync_at / last_error flow (success)
 *  T5: Deno.serve guarded by import.meta.main
 *  T6: G7 cost-cap halt
 */
...
function makeRequest(body: unknown): Request {
  return new Request('https://project.supabase.co/functions/v1/rag-federated-dailymed', {
    method: 'POST',
    headers: {
    ...
    from: vi.fn((table: string) => {
      ...
      if (table === 'rag_sources') {
        return {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
          maybeSingle: vi.fn().mockResolvedValue({ data: { id: 'dailymed-source-uuid' }, error: null }),
        };
      }
      if (table === 'rag_topics') {
        return {
...
const SPL_DETAIL = {
  ...
  sections: [
    ...
    { code: '34067-9', title: 'INDICATIONS', text: 'For type 2 diabetes.' },
  ],
};
...
describe('handleRequest (rag-federated-dailymed)', () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.useRealTimers();
  });

  it('T1: incremental → searchSPLs → getSPLDetail → INSERT rag_chunks', async () => {
    vi.mocked(searchSPLs).mockResolvedValue({ data: [{ setid: 'setid-001' }] });
    vi.mocked(getSPLDetail).mockResolvedValue(SPL_DETAIL);
    ...
    expect(searchSPLs).toHaveBeenCalledOnce();
    expect(getSPLDetail).toHaveBeenCalledWith('setid-001', expect.any(Object));
  });
  ...
  it('T3: duplicate content_hash → skipped_duplicate count incremented', async () => {
    vi.mocked(searchSPLs).mockResolvedValue({ data: [{ setid: 'setid-001' }] });
    vi.mocked(getSPLDetail).mockResolvedValue(SPL_DETAIL);
  ...
  it('T4: on success → updates last_sync_at', async () => {
  ...
  it('T5: import.meta.main guard prevents Deno.serve on module import', () => {
    expect(typeof handleRequest).toBe('function');
  });

  it('T6: G7 cost-cap fires emitCostEnvelopeBreach', async () => {
    vi.useFakeTimers();
    vi.setSystemTime(1000000000000);

    let callCount = 0;
    vi.mocked(searchSPLs).mockImplementation(async () => {
      callCount++;

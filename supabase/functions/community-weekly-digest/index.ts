/**
 * `community-weekly-digest` Edge Function — Phase 49 Plan 49-07 (DIGEST-03).
 *
 * Per-user weekly community digest. Invoked hourly by the pg_cron schedule
 ...
 *      (digest_course_progress_delta_7d, digest_upcoming_events_7d_rsvpd,
 *      digest_community_top3_7d). Each RPC takes `p_user_id` explicitly — the
 ...
 *      (user_id, category='weekly_community_digest', channel='email'):
 ...
 *      (Plan 49-08), category='weekly_community_digest'.
 *   8. Call `sendEmail({ template: 'community_weekly_digest', phi: false, ...,
 *      headers: { 'List-Unsubscribe': '<url>',
 *      'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click' } })`.
 *   9. UPSERT `digest_send_log` row with status='sent', kind='weekly'.
 *
...
interface WeeklyDigestContent {
  ...
  communityTop3: CommunityTop3Row[];
}
...
interface RunResult {
  ...
  bucket_counts?: { courseProgress: number; upcomingEvents: number; communityTop3: number };
  reason?: string;
}
...
/**
 * Call the 3 weekly-digest SECDEF RPCs concurrently. Each RPC takes
...
export async function computeWeeklyDigest(
  ...
  const [progressRes, eventsRes, top3Res] = await Promise.all([
    c.rpc('digest_course_progress_delta_7d', { p_user_id: userId }),
    c.rpc('digest_upcoming_events_7d_rsvpd', { p_user_id: userId }),
    c.rpc('digest_community_top3_7d', { p_user_id: userId }),
  ]);
  return {
    courseProgress: Array.isArray(progressRes?.data)
      ? (progressRes.data as CourseProgressRow[])
      : [],
    upcomingEvents: Array.isArray(eventsRes?.data)
      ? (eventsRes.data as UpcomingEventRow[])
      : [],
    communityTop3: Array.isArray(top3Res?.data)
      ? (top3Res.data as CommunityTop3Row[])
      : [],
  };
}

/** True iff every bucket is empty (no email-worthy content for this user). */
export function isEmpty(content: WeeklyDigestContent): boolean {
  return (
    content.courseProgress.length === 0 &&
    content.upcomingEvents.length === 0 &&
    content.communityTop3.length === 0
  );
}
...
async function upsertDigestLog(
  ...
  try {
    ...
    console.warn(
      '[community-weekly-digest] digest_send_log upsert failed',
...
/**
 * Check if the user has explicitly opted out of the weekly community digest
...
async function isOptedOut(
  ...
  try {
    const res = await c
      ...
      .select('enabled')
      .eq('user_id', userId)
      .eq('category', 'weekly_community_digest')
      .eq('channel', 'email')
      .maybeSingle();
    ...
    console.warn(
      '[community-weekly-digest] notification_settings lookup failed',
      e instanceof Error ? e.name : 'unknown',
    );
    return false;
  }
...
export async function handleRun(userId: string): Promise<RunResult> {
  ...
  let content: WeeklyDigestContent;
  try {
    content = await computeWeeklyDigest(supabase, userId);
  } catch (e) {
    await upsertDigestLog(supabase, userId, 'error', 'compute_failed');
    console.warn(
      '[community-weekly-digest] compute failed',
      e instanceof Error ? e.name : 'unknown',
    );
  ...
  const bucket_counts = {
    courseProgress: content.courseProgress.length,
    upcomingEvents: content.upcomingEvents.length,
    communityTop3: content.communityTop3.length,
  };
  ...
  try {
    ...
    console.warn(
      '[community-weekly-digest] mintUnsubscribeToken failed',
      e instanceof Error ? e.name : 'unknown',
    );
  ...
  try {
    ...
    console.warn(
      '[community-weekly-digest] sendEmail threw',
      e instanceof Error ? e.name : 'unknown',
    );
...
//
// Guarded by `COMMUNITY_WEEKLY_DIGEST_DISABLE_SERVE=1` so unit tests can import
...
if (Deno.env.get('COMMUNITY_WEEKLY_DIGEST_DISABLE_SERVE') !== '1') {
  Deno.serve(async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      console.warn(
        '[community-weekly-digest] unhandled',
        e instanceof Error ? e.name : 'unknown',
      );
